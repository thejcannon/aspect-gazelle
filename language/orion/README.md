...
*Args*:
- `name`: a unique identifier for the extension, may be referenced in Starlark API or used in `# gazelle:{name} enabled|disabled` directives etc
...
* `.properties`: a name:value map of extension property values configured in `BUILD` files via `# gazelle:{name} {value}`
